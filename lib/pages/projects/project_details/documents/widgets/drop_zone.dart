...
class _FileDropZoneState extends State<FileDropZone>
                        ...
                        child: Text.rich(
                          TextSpan(
                            children: [
                              ...
                              TextSpan(
                                text:
                                    ' ,faites glisser des fichiers depuis vote appareil ou ',
                                style: TextStyle(
                                    height: 1.5,
                                    color: text,
                                    fontSize: 12,
                              ...
                              TextSpan(
                                text:
                                    ",  puis insérez-les ici pour les ajouter aux documents.",
                                style: TextStyle(
                                    color: text,
                                    height: 1.5,
                                    fontSize: 12,
  ...
  Future acceptFile(dynamic value, BuildContext context) async {
    ...
    var bytes;
    bytes = await compute(controller.getFileData, value);
    Document document = new Document(new Random().nextInt(9999), file.name, "0",
        file.name, users.last, DateTime.now(), file.size, false);
    await bloc.documentUpload(document, bytes);
    _controller.reset();
    setState(() => highlight = false);
  }
