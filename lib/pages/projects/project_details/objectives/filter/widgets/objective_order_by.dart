...
class ObjectiveOrderBy extends StatefulWidget {
  final double widgetHeight;
  final double popupMenuWidth;
  const ObjectiveOrderBy({
    Key? key,
  ...
  @override
  _ObjectiveOrderByState createState() => _ObjectiveOrderByState();
}

class _ObjectiveOrderByState extends State<ObjectiveOrderBy> {
  String orderBy = objectiveOrderByAsText(objectiveListOrder.orderBy);
          ...
          return [
            CustomIconListPopupMenuItem(
                ...
                child: Container(
                    height: 161,
                    width: 150,
                    child:ObjectiveOrderByListView(onSelectPropriety: (String value) { setState(() {orderBy = value;}); },
...
class ObjectiveOrderByListView extends StatefulWidget {
  final Function(String value) onSelectPropriety;
  final Function(bool value) setAscending;
  const ObjectiveOrderByListView({Key? key,required this.onSelectPropriety , required this.setAscending}) : super(key: key);

  @override
  _ObjectiveOrderByListViewState createState() => _ObjectiveOrderByListViewState();
}

class _ObjectiveOrderByListViewState extends State<ObjectiveOrderByListView> {
  bool isAscending = objectiveListOrder.isAscending;
  @override
  Widget build(BuildContext context) {
      ...
      child: ListView(
          ...
          children: [
            ListTile(
              ...
              onTap: () async {
                objectiveListOrder.isAscending = true;
                setState(() {
                  isAscending = true;
                });
                bloc.fetch();
            ...
            ListTile(
              ...
              onTap: () async {
                objectiveListOrder.isAscending = false;
                setState(() {
                  isAscending = false;
                });
                bloc.fetch();
            ...
            ...ObjectiveOrderByProperties.values
                .map((e) {
              bool isChecked = objectiveListOrder.orderBy == e;
              return ListTile(
                contentPadding:
                ...
                onTap: () async {
                  widget.onSelectPropriety(objectiveOrderByAsText(e));
                  objectiveListOrder.orderBy = e;
                  setState(() {
                    isChecked = true;
                  });
                  bloc.fetch();
