...
List<Project> filteredList = Projects;
  ...
  Future init () async
  {
    await Future.delayed(Duration(milliseconds: 300) , ()=> _controller.sink.add(filter(Projects)));
  }
  fetch(){
  ...
  List<Project> filter(List<Project> initList)
  {
    ...
    switch (projectsFilterData.status){
      ...
      case Status.inProgress :
        result = result.where((project) => identical(project.status, Status.inProgress)).toList();
        break;
        case Status.completed :
          result = result.where((project) => identical(project.status, Status.completed)).toList();
          break;
    }
    if(projectsFilterData.startDate != null)
    result = result.where((project) => project.endDate.isBefore(projectsFilterData.endDate!) &&  project.endDate.isAfter(projectsFilterData.startDate!)).toList();
    
    if(projectsFilterData.types!=null)
      result.removeWhere((project) => projectsFilterData.types!.contains(project.type));
    if(projectsFilterData.users!=null)
      result.removeWhere((project) => projectsFilterData.users!.contains(project.teamLeader));

    result = result.where((project) => project.name.toLowerCase().contains(projectsSearchQuery.toLowerCase())).toList();
